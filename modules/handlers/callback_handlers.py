...
def course_callback(update: Update, context: CallbackContext) -> int:
    ...
    if url.find('category=') != -1 or len(url) <= 36:
        ...
        query.edit_message_text(text=f"Cannot get this path. Please /start again.")
        return STATE['end']
    
    context.user_data['last_studium_out'] = studium_out
    context.user_data['shift_menu_index'] = 0
    context.user_data['last_url'] = url
    context.user_data['base_url'] = url[:url.find('unict.it') + len('unict.it')]

    generate_keyboard(update=update.callback_query, context=context, studium_out=studium_out, message_text='Seleziona l\'anno accademico')
...
def shift_menu_index_down(update: Update, context: CallbackContext) -> None:
    ...
    generate_keyboard(update=update.callback_query, context=context, studium_out=context.user_data['last_studium_out'], message_text='Seleziona l\'anno accademico')
